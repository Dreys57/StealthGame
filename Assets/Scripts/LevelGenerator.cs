...
public class LevelGenerator : MonoBehaviour
{
    ...
    List<Room> DivisionByX(Room room)
    {
        ...
        rightRoom.center = new Vector2(room.center.x + room.extend.x * 0.5f - rightRoom.extend.x * 0.5f, room.center.y);
        rightRoom.childrenRooms = new List<Room>();
        
        leftRoom.extend = new Vector2(room.extend.x - posX, room.extend.y);
        leftRoom.center = new Vector2(room.center.x - room.extend.x * 0.5f + leftRoom.extend.x * 0.5f, room.center.y);
    ...
    List<Room> DivisionByY(Room room)
    {
        ...
        roomDown.center = new Vector2(room.center.x, room.center.y - room.extend.y * 0.5f + roomDown.extend.y * 0.5f);
        roomDown.childrenRooms = new List<Room>();
        
        roomUp.extend = new Vector2(room.extend.x, room.extend.y - posY);
        roomUp.center = new Vector2(room.center.x, room.center.y + room.extend.y * 0.5f - roomUp.extend.y * 0.5f);
    ...
    {
        Gizmos.DrawWireCube(room.center, room.extend);
